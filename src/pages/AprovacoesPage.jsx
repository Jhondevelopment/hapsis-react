      ...
      <motion.div initial={{opacity:0,y:16,scale:0.95}} animate={{opacity:1,y:0,scale:1}}
        ...
        <div style={{textAlign:'center',marginBottom:24}}>
          <div style={{width:56,height:56,borderRadius:'50%',background:`${corBotao}14`,
            ...
            justifyContent:'center',fontSize:26,margin:'0 auto 16px'}}>⚠️</div>
        ...
        <div style={{display:'flex',gap:10}}>
          <motion.button onClick={onCancelar} whileHover={{background:'rgba(255,255,255,0.10)'}} whileTap={{scale:0.97}}
            style={{flex:1,padding:'12px',background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.10)',
              borderRadius:10,color:'#8f94b0',fontWeight:600,fontSize:14,cursor:'pointer'}}>
            Cancelar
          </motion.button>
          <motion.button onClick={onConfirmar} whileHover={{filter:'brightness(1.10)'}} whileTap={{scale:0.97}}
            style={{flex:2,padding:'12px',background:`linear-gradient(160deg,${corBotao}dd,${corBotao}aa)`,
...
export function AprovacoesPage({ perfil }) {
  ...
  async function carregarStats() {
    try {
      const { data } = await supabase
        .from('leads').select('aprovado,estornado,valor').eq('status','Fechados')
      if (!data) return
      setStats({
    ...
    } catch(e) { console.error('stats erro:',e) }
  }
  ...
  async function carregar() {
    ...
    try {
      ...
      if      (filtroAtual==='pendentes')  q = q.or('aprovado.eq.false,aprovado.is.null').eq('estornado',false)
      else if (filtroAtual==='aprovados')  q = q.eq('aprovado',true).eq('estornado',false)
      else if (filtroAtual==='estornados') q = q.eq('estornado',true)
      ...
      console.error('carregar aprovações erro:',e)
    } finally {
      setLoading(false)
    }
  }
  ...
  async function executarAcao() {
    ...
    try {
      ...
      // Realtime vai atualizar automaticamente — não precisa chamar carregar() manualmente
    } catch(e) { console.error('ação erro:',e) }
  }

  const confirmConfigs = {
    aprovar:  { titulo:'Aprovar esta venda?',  mensagem:'A venda entrará no caixa imediatamente.',             corBotao:'#00c896', labelBotao:'✓ Aprovar' },
    rejeitar: { titulo:'Rejeitar esta venda?', mensagem:'A venda voltará para Perdidos.',                      corBotao:'#ff4d6a', labelBotao:'✕ Rejeitar' },
    estornar: { titulo:'Estornar esta venda?', mensagem:'Remove do caixa. Esta ação é irreversível.',          corBotao:'#ff8c42', labelBotao:'↩️ Confirmar Estorno' },
  }
  ...
  return (
    <div>
      <div style={{marginBottom:24}}>
        ...
        <p style={{fontSize:13,color:'#4c5070',marginTop:4}}>Atualiza automaticamente em tempo real</p>
      </div>
        ...
        {abas.map(s=>(
          <motion.div key={s.id} whileHover={{y:-2}} onClick={()=>setFiltro(s.id)}
            ...
            <p style={{fontFamily:'Syne,sans-serif',fontSize:28,fontWeight:700,color:s.cor}}>
              {s.n}
              {s.n>0&&s.id==='pendentes'&&(
                <motion.span animate={{opacity:[1,0.4,1]}} transition={{duration:1.5,repeat:Infinity}}
                  style={{display:'inline-block',width:8,height:8,borderRadius:'50%',background:s.cor,marginLeft:8,verticalAlign:'middle'}} />
              )}
            </p>
      ...
      {loading ? (
        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',height:260,
          ...
          <p style={{fontSize:14,fontWeight:600,color:'#4c5070'}}>
            {filtro==='pendentes'?'Nenhuma venda aguardando':filtro==='aprovados'?'Nenhuma aprovada ainda':'Nenhum estorno'}
          </p>
        </div>
      ) : (
              ...
              return(
                <motion.div key={lead.id}
                  ...
                  style={{padding:'18px 20px',borderRadius:14,
                    ...
                    backdropFilter:'blur(28px)',border:`1px solid ${corBorda}33`,
                    borderLeft:`3px solid ${corBorda}`,position:'relative',overflow:'hidden'}}>
                  <div style={{position:'absolute',top:0,left:0,right:0,height:1,
                    background:`linear-gradient(90deg,transparent,${corBorda}22,transparent)`}} />

                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:14}}>
                    <div style={{flex:1}}>
                      {/* Cliente + valor */}
                      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10,flexWrap:'wrap'}}>
                        <div style={{width:42,height:42,borderRadius:'50%',background:`${corBorda}18`,
                          border:`1px solid ${corBorda}30`,display:'flex',alignItems:'center',
                          justifyContent:'center',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:16,color:corBorda,flexShrink:0}}>
                      ...
                      <div style={{display:'inline-flex',alignItems:'center',gap:7,padding:'5px 12px',
                        background:'rgba(157,111,255,0.08)',border:'1px solid rgba(157,111,255,0.18)',
                        borderRadius:8,marginBottom:10}}>
                        <div style={{width:20,height:20,borderRadius:'50%',background:'rgba(157,111,255,0.20)',
                          display:'flex',alignItems:'center',justifyContent:'center',fontSize:11,fontWeight:700,color:'#9d6fff'}}>
                      ...
                      {(lead.comprovante_url||lead.contrato_url||lead.doc_importante_url)&&(
                        <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:10}}>
                          {lead.comprovante_url&&(
                            <a href={lead.comprovante_url} target="_blank" rel="noreferrer"
                              style={{display:'inline-flex',alignItems:'center',gap:5,padding:'6px 12px',
                                ...
                                borderRadius:7,color:'#00c896',fontSize:12,fontWeight:600,textDecoration:'none'}}>
                              💳 Comprovante
                            </a>
                          )}
                          {lead.contrato_url&&(
                            <a href={lead.contrato_url} target="_blank" rel="noreferrer"
                              style={{display:'inline-flex',alignItems:'center',gap:5,padding:'6px 12px',
                                background:'rgba(77,159,255,0.10)',border:'1px solid rgba(77,159,255,0.22)',
                                borderRadius:7,color:'#4d9fff',fontSize:12,fontWeight:600,textDecoration:'none'}}>
                              📄 Contrato
                            </a>
                          )}
                        </div>
                      ...
                      {filtro==='pendentes'&&!lead.comprovante_url&&(
                        <div style={{padding:'7px 12px',borderRadius:8,background:'rgba(240,180,41,0.06)',
                          border:'1px solid rgba(240,180,41,0.18)',marginBottom:8}}>
                          <p style={{fontSize:11.5,color:'#f0b429',margin:0}}>⚠️ Nenhum comprovante anexado</p>
                        </div>
                      )}
                    ...
                    <div style={{display:'flex',flexDirection:'column',gap:8,flexShrink:0,minWidth:120}}>
                        ...
                        <>
                          <motion.button onClick={()=>setConfirm({tipo:'aprovar',leadId:lead.id})}
                            ...
                            style={{padding:'10px 0',width:'100%',
                              background:'linear-gradient(180deg,rgba(0,200,150,0.20),rgba(0,200,150,0.10))',
                              border:'1px solid rgba(0,200,150,0.32)',borderRadius:9,
                          ...
                          <motion.button onClick={()=>setConfirm({tipo:'rejeitar',leadId:lead.id})}
                            ...
                            style={{padding:'10px 0',width:'100%',background:'rgba(255,77,106,0.10)',
                              border:'1px solid rgba(255,77,106,0.26)',borderRadius:9,
                              color:'#ff4d6a',fontWeight:700,fontSize:13,cursor:'pointer',transition:'all 0.15s'}}>
                            ✕ Rejeitar
                          </motion.button>
                        </>
                      )}
                      {filtro==='aprovados'&&(
                        <motion.button onClick={()=>setConfirm({tipo:'estornar',leadId:lead.id})}
                          whileHover={{y:-1}} whileTap={{scale:0.97}}
                          style={{padding:'10px 0',width:'100%',background:'rgba(255,140,66,0.10)',
                            border:'1px solid rgba(255,140,66,0.26)',borderRadius:9,
                            color:'#ff8c42',fontWeight:700,fontSize:13,cursor:'pointer',transition:'all 0.15s'}}>
                          ↩️ Estornar
                        </motion.button>
                      )}
                    </div>
